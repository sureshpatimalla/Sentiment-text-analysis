...
aurl <- "https://www.amazon.in/product-reviews/B07DJHXTLJ/ref=acr_dpproductdetail_text?ie=UTF8&showViewpoints=1"
amazon_reviews <- NULL
library(htmltools)
...
write.csv(amazon_reviews,"amazon.csv",row.names = F)
getwd()


###################################(Sentimental_analysis_Oneplus)###################################################################################################################################
...
x = readLines(file.choose()) 	# first, read-in data from "amazon_reviews,"oneplus.txt""
